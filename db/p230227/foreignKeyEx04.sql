...
create table if not exists buy(
    ...
    foreign key (id) references customer(id) on delete no action on update no action
);
...
# no action 설정
set foreign_key_checks = 0; # 0 = false, 1 = true / 기본값은 1
							# 값이 1일 때 자식이 있다면 부모쪽에 데이터가 있는지 검사하고 있다면 에러
set foreign_key_checks = 1;                            

# [삭제] customer 테이블의 id가 1002인 row 삭제
delete from customer where id=1002;					# on delete no action on update no action 상태 / 기본 상태
													# set foreign_key_checks = 0; 설정으로 부모에 값이 있는지 검사하지 않아서 실행

# [수정] customer 테이블의 id가 1000인 row의 id값을 2000으로 수정
update customer										# on delete no action on update no action 상태 / 기본 상태
set id = 2000
where id = 1000;
