...
create table if not exists buy(
    ...
    foreign key (id) references customer(id) on delete set null on update set null
);
...
delete from customer where id=1002;					# on delete set null on update set null 상태

# [수정] customer 테이블의 id가 1000인 row의 id값을 2000으로 수정
update customer										# on delete set null on update set null 상태
set id = 2000
where id = 1000;
