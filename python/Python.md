      ...
      ● [분석]
        ...
        ● [시각화]
            ...
            ● [차트]
              ...
              ○ matplotlib.pyplot.legend()
                ...
                - [매개변수]
                  ...
                  - ncols : 컬럼의 개수 지정
                  - bbox_to_anchor : 범례 생성 위치 지정
                    - bbox_to_anchor가 먼저 위치를 지정하고 loc이 적용
                    - 생성 위치를 기준으로 loc의 값으로 뒤집음
                      ex) upper right -> 위에서 아래로 오른쪽에서 왼쪽으로 뒤집음
          ...
          ● [seaborn]
            ...
            - 기본적으로 그래프 그리는 방식은 평균으로 그리고 데이터의 편차를 표현
            ...
            ○ seaborn.lineplot(데이터, x, y, hue)
              ...
              - [매개변수]
                ...
                - hue : 그룹화 기준
                - errorbar : 데이터의 편차
                - estimator : 그래프 그리는 방식
                  - numpy의 집계함수를 사용
                - palette : seaborn의 colormap
                - order : x축 카테고리 설정
                  - 특정 값만 볼 수도 있고, 정렬을 할 수도 있음

              ○ seaborn.barplot(데이터, x, y, hue)
                - 막대 그래프

              ○ seaborn.kdeplot(데이터)
                - KDE(Kernel Density Estimation) : 커널 밀도 추정 그래프
                - 정규분포 그래프 중 하나
                - 그래프의 모양으로 이상치 유무 판단 가능
                  - 종 모양 : 이상적인, 정상적인 분포
                  - 꼬리가 늘어진 그래프 : 오른쪽으로 편향된 분포(오른쪽에 이상치 존재)
                    - Positive Skewness
                    - 균값을 오른쪽으로 치우침
                    - 평균값이 대부분의 평균값보다 작음
                  - 꼬리가 늘어진 그래프 : 왼쪽으로 편향된 분포(왼쪽에 이상치 존재)
                    - Negative Skewness
                    - 평균값을 왼쪽으로 치우침
                    - 평균값이 대부분의 평균값보다 큼
              ...
              ○ seaborn.boxplot(데이터, x, y, hue)
                - 박스 그래프
                  - 박스 하단 : 1사분위
                  - 박스 안 초록선 : 2사분위(중앙값)
                  - 박스 상단 : 3사분위
                  - 상단 하단 선 : 1사분위 3사분위의 1.5배 값(상위, 하위 경계 / 이상치가 아닌 범위)
                  - 검은선 : 최대, 최소값
                    - 최대값과 최소값은 완전히 보장하진 못 함
                  - 아웃라이어 : 이상치, 극단값

              ○ seaborn.violinplot(데이터, x, y, hue)
                - 바이올린 그래프
                  - 가운데 검은색 - 박스 그래프
                  - 주위 범위 - 데이터의 분포
                  - 한쪽으로 길게 나온 그래프는 해당 위치에 이상치 존재

            ○ 그래프.figure.set_size_inches()
              - 그래프 크기 조정

      ● [전처리]
        - 외부 데이터에서 불필요한 정보를 걸러내거나 필요한 정보를 추가하는 과정
        - 데이터를 수정하기 때문에 데이터의 크기도 변경됨

      ● [편향]
        - 데이터에 이상치가 존재하는 상태
        - 왜도(skewness)라고 불리기도 함
          - 이상치(극단값) : 관측된 데이터의 범위에서 많이 벗어난 값

          - 평균에 영향을 주기 때문에 처리해주는 것이 좋다
            - 삭제
            - 그대로 사용
            - 값 변경
            - 평균으로 변경
  ...
  [AI]
    ...
    - ML(Machine Learning) : 기계를 학습시킬 방법
      - [전처리] : ML의 데이터는 문자 데이터를 취급하지 않기 때문에 문자 데이터를 숫자 데이터로 바꾸는 과정이 필요

      - [훈련(학습)] : 컴퓨터가 주어진 데이터에서 패턴을 분석해 최적의 방정식을 만드는 과정
        - underfitting : 학습이 부족한 상태 
        - overfitting : 학습이 과한 상태(추가로 다른 데이터가 들어오면 예측을 하지 못함)

      - [지도학습] : 답이 정해져 있는 문제를 학습
        - 분류 : 데이터의 패턴을 분석해 데이터를 분류
          - 분류에서 target(답)을 label or class라 함
          - label or class를 제외한 모든 것을 feature라 함

        - 회귀 : 데이터의 패턴을 분석해 결과를 예측
          - 선형회귀
            - y=Wx + b
            - x : 독립변수(입력값)
            - y : 종속변수(결과값)
      ...
      - [정확도(Accuracy)] : 실제 데이터에서 예측 데이터가 얼마나 같은지를 판단하는 지표
        ...
        - 평균 제곱근 오차(RMSE : Root Mean Square Error) : 오차의 제곱 합의 평균의 제곱근
            ...
            - 1은 괜찮은 수준, 10은 훈련이 잘 안된 수준      

        - 정확도가 모델 선택의 절대적인 척도가 되진 않음, 정확도가 더 떨어지는 모델이라도 사용하기 더 좋다 판단되면 해당 모델을 사용하기도 함
    ...
    - 데이터를 통해 학습하기 때문에 데이터에 의존적인 학습 방법

    - 데이터 셋 분리(scikit-learn)
      - Train data
          - 모델을 학습시키기 위한 데이터 셋
          - 학습은 최적의 파라미터를 찾는 것
          - 학습을 위한 데이터
    ...
    ● [교차검증]
      - 데이터를 n등분해 테스트에 쓸 데이터와 훈련에 쓸 데이터의 비율을 정해 등분을 바꿔가며 검증
      
      ● [KFold]
      - 데이터가 들어가 있는 순서대로 세트를 분리(셔플 가능)
      - 회귀에 주로 사용

        ○ KFold(n_splits)
          - n_splits만큼 등분할 폴드 객체 생성

        ○ KFold.split(데이터)
          - 입력한 데이터를 n등분한 폴드 세트 생성
          - 반복문을 통해 접근 가능
          - 리턴 : 등분한 인덱스 리턴
      ...
      ● [Stratified KFold]
        ...
        - 분류에 주로 사용
          ...
          ○ StratifiedKFold.split(데이터, 기준 컬럼)
            - 입력한 기준 컬럼으로 입력한 데이터를 n등분한 폴드 세트 생성
            - 반복문을 통해 접근 가능
            - 리턴 : 등분한 인덱스 리턴
    ...
    ● [scikit-learn]
      ...
      ○ sklearn.model_selection.train_test_split(데이터)
        - 훈련용(train) 데이터, 검증용(test) 데이터를 나눠주는 함수

        - [매개변수] 
          - 데이터 : numpy의 배열, pandas의 데이터프레임, 리스트
            - feature : 속성, 변수, 입력값
            - target, label : 결과값
          - test_size : 테스트용 데이터의 비율
            - 소수로 입력(0~1)
          - random_state : 정수값(random seed)을 기준으로 데이터 분리
            - 항상 일정한 결과로 데이터 분리
          - shuffle : 데이터를 분리하기 전에 데이터를 섞음
          - stratify(계층) : 설정한 데이터를 기준으로 데이터를 분리해 편향되지 않게 설정

      ○ sklearn.datasets.load_X
        - 사이킷런이 저장한 데이터셋을 사용
        - 타입이 실제는 dict이지만 클래스에서 설정한 타입인 Bunch로 나옴

      ○ sklearn.tree.DecisionTreeClassifier()
        - 결정트리분류모델클래스
        - 트리 기반의 분류 규칙을 만드는 알고리즘
          - 노드 : 
          - 최상위 노드(Root Node) : 시작점
          - 최하단 노드(Leaf Node) : 결정된 클래스 값
          - 규칙노드(Decision Node) : 규칙 조건
          - 깊이(depth) : 트리의 깊이(분류의 수)
            - 깊이가 너무 깊으면 처리해야될 조건이 늘어나기 때문에 성능저하가 발생(overfitting)

        ● [파라미터]
          - 모델 내부에서 결정된 값, 데이터로 결정되는 값
          
          - [파라미터]
            - random_status
      
        ● [하이퍼 파라미터]
          - 예측의 성능을 높이기 위해 사용자가 설정하는 값
        ...
        ○ DecisionTreeClassifier.fit(학습용 feature, 학습용 target)
          - 학습용 feature와 target을 통해 학습 수행
            - feature : 속성, 변수, 입력값
            - target, label : 결과값

        ○ DecisionTreeClassifier.predict(테스트용 feature)
          - 학습 수행한 결과를 바탕으로 문제의 답을 예측

      ○ sklearn.metrics.accuracy_score(테스트용 target, 예측값)
        - 정확도 평가(예측값이 정답과 얼마나 같은지 비교)
        - 이용이 쉽지만 정확도 구하는 과정을 알기 위해선 라이브러리의 함수를 뜯어서 확인해야 함
